...
#include <stdio.h>
#include <stdlib.h>
#define FLASHVOLUM 32   //Flash的容量，以K为单位
...
void main()
{
    ...
    PrintFlash(flashbuffer,16*1024);//仅显示前2K的内容
    WriteFlashToFile(flashbuffer,"main_formated.txt",16*1024);//将前2K的内容写入文件"main_formated.txt"中
    //释放内存
    free(buffer);
    free(trsbuffer);
    free(flashbuffer);
