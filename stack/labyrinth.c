...
int main()
{
    ...
    start.x=1;start.y=1;//迷宫的起点
    end.x=8;end.y=8;//迷宫的终点
    InitMaze(&maze);//迷宫初始化
    printf("迷宫形状：\n");
    ...
    do{
        if(TRUE==Pass(&maze,curpos)){
            ...
            if((curpos.x==end.x)&&(curpos.y==end.y)){//到达终点（出口）
                printf("迷宫路径:\n");
                //StackTraverse(S); //遍历顺序栈，得到最短路径坐标
                PrintFoot(&maze,S);
                return TRUE;
            }
...
Status FootPrint(MazeType *M,PosType pos){
    //将迷宫的当前位置pos设置为"走过"，即footprint该位置为1
    if((pos.x>M->row-1)||(pos.y>M->column-1))
        return FALSE;
...
Status Pass(MazeType *M,PosType pos){
    //判断当前位置是否可通，即为未走过的通道块
    if((M->row-1<pos.x)||(M->column-1<pos.y)){
        printf("位置越位.\n");
        exit(0);
    }
